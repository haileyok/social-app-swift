...
public enum SearchRowData {
  ...
  public static func avatar(_ profile: App.Bsky.ActorDefs_ProfileView) -> AvatarSource {
  ...
  public static func avatar(_ profile: App.Bsky.ActorDefs_ProfileViewBasic) -> AvatarSource {
    AvatarSource.resolve(
      avatar: profile.avatar?.rawValue,
      handle: profile.handle.rawValue,
      displayName: profile.displayName)
  ...
  public static func displayName(_ profile: App.Bsky.ActorDefs_ProfileView) -> String {
    let name = profile.displayName?.trimmingCharacters(in: .whitespacesAndNewlines)
    return (name?.isEmpty == false ? name! : "@\(profile.handle.rawValue)")
  }

  /// The name a basic-profile row shows.
  public static func displayName(_ profile: App.Bsky.ActorDefs_ProfileViewBasic) -> String {
    let name = profile.displayName?.trimmingCharacters(in: .whitespacesAndNewlines)
    return (name?.isEmpty == false ? name! : "@\(profile.handle.rawValue)")
  }

  /// The handle line, with the leading `@` the RN rows use.
  public static func handle(_ profile: App.Bsky.ActorDefs_ProfileView) -> String {
    "@\(profile.handle.rawValue)"
  }

  /// The bio line, when the profile has one.
  public static func bio(_ profile: App.Bsky.ActorDefs_ProfileView) -> String? {
  ...
  public static func title(_ feed: App.Bsky.FeedDefs_GeneratorView) -> String {
    feed.displayName
  }

  /// The feed's byline.
  public static func byline(_ feed: App.Bsky.FeedDefs_GeneratorView) -> String {
    SearchCopy.feedByline(creatorHandle: feed.creator.handle.rawValue)
  }

  /// The feed's description, when it has one.
  public static func description(_ feed: App.Bsky.FeedDefs_GeneratorView) -> String? {
  ...
  public static func title(_ pack: App.Bsky.GraphDefs_StarterPackView) -> String {
    pack.list?.name ?? SearchCopy.starterPackTitleFallback
  }

  /// The starter pack's byline.
  public static func byline(_ pack: App.Bsky.GraphDefs_StarterPackView) -> String {
  ...
  public static func title(_ topic: App.Bsky.UnspeccedDefs_TrendingTopic) -> String {
    topic.displayName ?? topic.topic
  }

  /// A trending video's display name.
  public static func title(_ trend: App.Bsky.UnspeccedDefs_TrendView) -> String {
    trend.displayName
  }

  /// A trending video's post-count caption.
  public static func postCount(_ trend: App.Bsky.UnspeccedDefs_TrendView) -> String {
...
private extension UnknownATPValue {
  var postRecord: App.Bsky.FeedPost? {
