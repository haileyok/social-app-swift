// TLD data for `isValidDomain`, vendored from the `tlds` npm package via
// `tools/golden-gen`, which pins `@bsky/sdk@1.1.0` - the same version the RN app
// uses. Do not hand-edit the list below; it is generated data.
//
// The npm package is a plain JSON list, not code, so only the data is vendored.
// There is deliberately no `test` entry: `detection.js` accepts `.test` handles
// through a separate explicit check.
//
// Regenerate with:
//   cd tools/golden-gen && node -e "const t=require('tlds'); \
//     for (let i=0;i<t.length;i+=8) console.log('  ' + \
//     t.slice(i,i+8).map(x=>JSON.stringify(x)).join(', ') + ',')"

/// Whether `str` ends in a known TLD, i.e. has a `.`-preceded final label that
/// is a registered TLD.
///
/// Ported from `isValidDomain` (`@bsky/sdk/richtext/detection.js` and the RN
/// app's `url-helpers.ts`, which are identical). Note this is a TLD *membership*
/// check, not a general domain validator: it accepts any string whose final
/// label is a known TLD, including leading garbage like `!!.com`. Callers rely
/// on the detection regexes to bound what reaches it.
public func isValidDomain(_ string: String) -> Bool {
  guard let dot = string.lastIndex(of: "."), dot < string.index(before: string.endIndex) else {
    return false
  }
  // Deliberately case-sensitive, like the original: the TLD list is lowercase
  // and the original compares raw substrings, so "EXAMPLE.COM" is not valid.
  return knownTLDs.contains(String(string[string.index(after: dot)...]))
}

/** IANA TLD labels. 1438 entries. */
let knownTLDs: Set<String> = [
  ...
  "gs", "gt", "gu", "gucci", "guge", "guide", "guitars", "guru",
  ...
  "jll", "jm", "jmp", "jnj", "jo", "jobs", "joburg", "jot",
  "joy", "jp", "jpmorgan", "jprs", "juegos", "juniper", "kaufen", "kddi",
  "ke", "kerryhotels", "kerryproperties", "kfh", "kg", "kh", "ki", "kia",
  ...
  "voto", "voyage", "vu", "wales", "walmart", "walter", "wang", "wanggou",
  "watch", "watches", "weather", "weatherchannel", "webcam", "weber", "website", "wed",
  ...
  "zw", "ελ", "ευ", "бг", "бел", "дети", "ею", "католик",
  "ком", "мкд", "мон", "москва", "онлайн", "орг", "рус", "рф",
  "сайт", "срб", "укр", "қаз", "հայ", "ישראל", "קום", "ابوظبي",
  "ارامكو", "الاردن", "البحرين", "الجزائر", "السعودية", "العليان", "المغرب", "امارات",
  "ایران", "بارت", "بازار", "بيتك", "بھارت", "تونس", "سودان", "سورية",
  "شبكة", "عراق", "عرب", "عمان", "فلسطين", "قطر", "كاثوليك", "كوم",
  "مصر", "مليسيا", "موريتانيا", "موقع", "همراه", "پاکستان", "ڀارت", "कॉम",
  "नेट", "भारत", "भारतम्", "भारोत", "संगठन", "বাংলা", "ভারত", "ভাৰত",
  "ਭਾਰਤ", "ભારત", "ଭାରତ", "இந்தியா", "இலங்கை", "சிங்கப்பூர்", "భారత్", "ಭಾರತ",
  "ഭാരതം", "ලංකා", "คอม", "ไทย", "ລາວ", "გე", "みんな", "アマゾン",
  "クラウド", "グーグル", "コム", "ストア", "セール", "ファッション", "ポイント", "世界",
  "中信", "中国", "中國", "中文网", "亚马逊", "企业", "佛山", "信息",
  "健康", "八卦", "公司", "公益", "台湾", "台灣", "商城", "商店",
  "商标", "嘉里", "嘉里大酒店", "在线", "大拿", "天主教", "娱乐", "家電",
  "广东", "微博", "慈善", "我爱你", "手机", "招聘", "政务", "政府",
  "新加坡", "新闻", "时尚", "書籍", "机构", "淡马锡", "游戏", "澳門",
  "点看", "移动", "组织机构", "网址", "网店", "网站", "网络", "联通",
  "谷歌", "购物", "通販", "集团", "電訊盈科", "飞利浦", "食品", "餐厅",
  "香格里拉", "香港", "닷넷", "닷컴", "삼성", "한국",
]
